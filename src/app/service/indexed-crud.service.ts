...
import { Earthquake } from '../interface/env';
...
export class IndexedCrudService{
...
loadData(dataUrl:string) : Promise<void> {
        return new Promise(resolve => {
            const response = this.http.get(dataUrl, {responseType:'text'}).pipe(map(data=>parse<{
                id:string, time: string, place : string, mag: string, depth : string, latitude : string, longitude : string
            }>(data, {header:true})));

            console.log('Response Kavish -->>',response);
            response.subscribe((res:any)=>{
                const tx = this.db.transaction('kavish_soam','readwrite');
                const store = tx.objectStore('kavish_soam');
    ...
    deleteOldRecords(): Promise<void> {
        ...
        const thirtyDaysAgo = Date.now() - IndexedCrudService.THIRTY_DAYS;
      ...
      initProvider(): Promise<void> {
        ...
        if (lastUpdate) {
          ...
          const now = Date.now();
          if (lastUpdateTs + IndexedCrudService.SEVEN_DAYS < now) {
          ...
          } else if (lastUpdateTs + IndexedCrudService.ONE_DAY < now) {
          ...
          } else if (lastUpdateTs + IndexedCrudService.ONE_HOUR < now) {
          ...
          } else if (lastUpdateTs + IndexedCrudService.FOURTYFIVE_MINUTES < now) {
    ...
    filter(filter: Filter): Promise<Earthquake[]> {
        const tx = this.db.transaction('kavish_soam', 'readonly');
        const store = tx.objectStore('kavish_soam');
      ...
      private initDb(): Promise<void> {
        ...
        return new Promise(resolve => {
          ...
          openRequest.onupgradeneeded = event => {
            const target: EventTarget | null = event.target;
            const db = (target as any).result;
            const store = db.createObjectStore('kavish_soam', {keyPath: 'id'});
            store.createIndex('mag', 'mag');
            store.createIndex('time', 'time');
          };
